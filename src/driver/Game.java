...
//Display board
        ...
        while (Board.getForest().size() > 0) {
            ...
            try {
                ...
                switch (option) {
                    case 1:
                        int position =
                                Integer.parseInt(keyboard.readLine("Enter the number of the card you want to take:"));
                        if
                        (currentPlayer.takeCardFromTheForest(position)) {
                            if
                            (Board.getForestCardsPile().pileSize() > 0) {
                                Board.getForest().add(Board.getForestCardsPile().drawCard());
                            }
                            succesfullMove = true;
                        }
                    ...
                    case 3:
                        ...
                        for (int k = 0; k < splittedStringOfInts.length; k++) {
                            int
                                    inputInt = Integer.parseInt(splittedStringOfInts[k]);
                            cookingmushrooms.add(currentPlayer.getHand().getElementAt(inputInt - 1));
                        }
                        if
                        (currentPlayer.cookMushrooms(cookingmushrooms)) {
                            succesfullMove = true;
                        }
                        break;
                    case 4:
                        ...
                        int mushNumber =
                                Integer.parseInt(keyboard.readLine("How many are willing to sell? Type number:"));
                        if
                        (currentPlayer.sellMushrooms(mushType, mushNumber)) {
                            succesfullMove = true;
                        }
                        break;
                    case 5:
    ...
    private static void printTxtDisplayable(Displayable d) {
        ...
        for (int k = 0; k < d.size(); k++) {
            if
            (d.getElementAt(k).getType().equals(CardType.NIGHTMUSHROOM)) {
                System.out.print("| (N) |");
            } else {
                System.out.print("| |");
            }
    ...
    private static void printTxtForest(CardList cl) {
        ...
        for (int k = 0; k < cl.size(); k++) {
            if
            (cl.getElementAt(k).getType().equals(CardType.NIGHTMUSHROOM)) {
                System.out.print("| (N) |");
            } else {
                System.out.print("| |");
            }
